...
function CopyModal({ item, type, onClose, onSave }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(4px)' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-surface)', borderRadius: 20, width: 400, padding: 28, boxShadow: '0 24px 80px rgba(0,0,0,0.25)' }}>
...
function RolesTab() {
  ...
  return (
    <div style={{ display: 'flex', gap: 18, minHeight: 0, flex: 1, overflow: 'hidden', width: '100%' }}>
              ...
              <div>
                ...
                {editingDesc ? (
                  ...
                  <div style={{ position: 'relative', minHeight: 120, padding: '12px 14px', background: 'var(--bg-subtle)', borderRadius: 10, border: '1.5px solid var(--border-light)', fontSize: 13, color: role.description ? 'var(--text-primary)' : 'var(--text-muted)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
                    {role.description || 'No description yet. Click the edit icon to add one.'}
      ...
      {editingRole && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setEditingRole(null)}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-surface)', borderRadius: 20, width: 440, padding: 28, boxShadow: '0 24px 80px rgba(0,0,0,0.25)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
...
function TagsAndCategoriesTab({ managementMode = false }) {
  ...
  const [tags, setTags] = useState(isAdminA ? [] : INITIAL_TAGS);
  const [cats, setCats] = useState(isAdminA ? [] : INITIAL_CATS);
